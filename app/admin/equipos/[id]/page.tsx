...
type JugadorDocumentos = {
  fotoUrl: string | null;
  dniDelanteUrl: string | null;
  dniDetrasUrl: string | null;
  dniTutorDelanteUrl: string | null;
  dniTutorDetrasUrl: string | null;
  esMenor: boolean;
};

function DocImagen({ titulo, url }: { titulo: string; url: string | null }) {
  return (
      ...
      {url ? (
        <a href={url} target="_blank" rel="noreferrer" className="block">
          ...
          <img
            src={url}
            alt={titulo}
            className="max-h-56 w-full rounded-lg border border-slate-200 bg-white object-contain"
          />
        </a>
      ) : (
...
export default function AdminEquipoDetallePage() {
  ...
  const [jugadorDocsAbiertoId, setJugadorDocsAbiertoId] = useState<string | null>(null);
  const [jugadorDocs, setJugadorDocs] = useState<Record<string, JugadorDocumentos>>({});
  const [cargandoDocsId, setCargandoDocsId] = useState<string | null>(null);
  ...
  async function onToggleDocumentosJugador(jugadorId: string) {
    if (jugadorDocsAbiertoId === jugadorId) {
      setJugadorDocsAbiertoId(null);
      return;
    }
    setJugadorDocsAbiertoId(jugadorId);
    if (jugadorDocs[jugadorId]) return;

    setCargandoDocsId(jugadorId);
    setMessage("");
    try {
      const {
        data: { session },
      ...
      const res = await fetch(`/api/admin/jugadores/${jugadorId}/documentos`, {
        headers: { Authorization: `Bearer ${session.access_token}` },
      });
      const json = (await res.json()) as JugadorDocumentos & { error?: string };
      if (!res.ok) {
        setMessage(`No se pudieron cargar los documentos: ${json.error ?? res.statusText}`);
        setJugadorDocsAbiertoId(null);
        return;
      }
      setJugadorDocs((prev) => ({ ...prev, [jugadorId]: json }));
    } catch {
      setMessage("Error de conexion al cargar DNI y foto.");
      setJugadorDocsAbiertoId(null);
    } finally {
      setCargandoDocsId(null);
    }
  }
  ...
  async function onDescargarPdfConsentimiento(jugadorId: string) {
    ...
    try {
      ...
      setMessage("PDF de inscripcion descargado.");
    } finally {
              ...
              {jugadores.length === 0 ? (
                ...
                <div className="grid gap-2">
                  {jugadores.map((jugador) => {
                    const docs = jugadorDocs[jugador.id];
                    const docsAbiertos = jugadorDocsAbiertoId === jugador.id;
                    return (
                      <div
                        key={jugador.id}
                      ...
                      >
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          ...
                          <div className="flex shrink-0 flex-wrap gap-2">
                            <button
                              ...
                              onClick={() => void onToggleDocumentosJugador(jugador.id)}
                              type="button"
                              disabled={cargandoDocsId === jugador.id}
                            >
                              {cargandoDocsId === jugador.id
                                ? "Cargando..."
                                : docsAbiertos
                                  ? "Ocultar DNI y foto"
                                  : "Ver DNI y foto"}
                        ...
                        {docsAbiertos ? (
                          <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50/50 p-4">
                            {cargandoDocsId === jugador.id ? (
                              <p className="text-sm text-slate-700">Cargando documentos...</p>
                            ) : docs ? (
                              <div className="grid gap-4 sm:grid-cols-2">
                                <DocImagen
                                  titulo="Foto de perfil"
                                  url={docs.fotoUrl ?? jugador.foto_url}
                                />
                                <DocImagen titulo="DNI delante" url={docs.dniDelanteUrl} />
                                <DocImagen titulo="DNI detras" url={docs.dniDetrasUrl} />
                                {docs.esMenor ? (
                                  <>
                                    <DocImagen
                                      titulo="DNI tutor delante"
                                      url={docs.dniTutorDelanteUrl}
                                    />
                                    <DocImagen titulo="DNI tutor detras" url={docs.dniTutorDetrasUrl} />
                                  </>
                                ) : null}
                              </div>
                            ) : (
                              <p className="text-sm text-slate-700">
                                No se pudieron mostrar los documentos.
                              </p>
                            )}
                          </div>
                        ) : null}
