...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const { data: row, error: qErr } = await admin
    .from("jugadores")
    .select(
      "id,nombre,apellidos,alias,fecha_nacimiento,es_menor,consentimiento_firma,consentimiento_aceptado_at,consentimiento_ip,consentimiento_user_agent,consentimiento_legal_version,consentimiento_legal_texto,tutor_email,tutor_telefono,tutor_dni,tutor_correo_enviado_at,equipos(nombre)",
    )
    .eq("id", jugadorId)
    .maybeSingle();
