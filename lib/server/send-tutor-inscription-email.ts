...
function buildMail(args: Args): { subject: string; html: string } {
  const legal = escapeHtml(args.legalFullText).replace(/\n/g, "<br/>");
...
<body style="font-family:system-ui,sans-serif;max-width:640px;margin:0 auto;padding:16px;color:#111">
  ...
  <p>Se ha completado la inscripcion de un menor en el torneo. A continuacion consta la informacion registrada y el texto legal aceptado por el tutor.</p>
  <table style="width:100%;border-collapse:collapse;font-size:14px;margin:16px 0">
    ...
    <tr><td style="padding:6px 0;border-bottom:1px solid #eee"><strong>Jugador/a (menor)</strong></td><td style="padding:6px 0;border-bottom:1px solid #eee">${escapeHtml(`${args.jugadorNombre} ${args.jugadorApellidos}`)}</td></tr>
    <tr><td style="padding:6px 0;border-bottom:1px solid #eee"><strong>DNI/NIE tutor (declarado)</strong></td><td style="padding:6px 0;border-bottom:1px solid #eee">${escapeHtml(args.tutorDni || "—")}</td></tr>
    ...
    <tr><td style="padding:6px 0;border-bottom:1px solid #eee"><strong>Versión texto legal</strong></td><td style="padding:6px 0;border-bottom:1px solid #eee">${escapeHtml(args.legalVersion)}</td></tr>
  </table>
  ...
  <h2 style="font-size:15px;margin-top:24px">${escapeHtml(args.legalTitle)}</h2>
 ...
 * Usa el texto legal exacto guardado en BD (mismo que firmo en pantalla).
 */
